...
int connector_helper_detect_ctx(struct drm_connector *connector, struct drm_modeset_acquire_ctx *ctx, bool force)
{
	...
	printk(KERN_INFO "gpio status: %d\n", status);
	if(status) {
...
{
    struct drm_display *drm = spi_get_drvdata(p);
	drm->connector.status = drm_helper_probe_detect(&drm->connector, NULL, false);
	drm_connector_helper_hpd_irq_event(&drm->connector);
    return IRQ_HANDLED;
}
...
static int drm_st7789v_probe(struct spi_device *spi)
{
	...
	check_irq = gpiod_to_irq(drm_st7789v->detect);
    //这里采用下半部分来发送热插拔消息，
    ret = request_threaded_irq(check_irq, NULL, check_irq_thread, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "drm hotplug", spi);
    if (ret) {
        dev_err(dev, "fail to request irq \n");
        gpiod_put(drm_st7789v->detect);
        return ret;
    }
    return 0;
}
...
static void drm_st7789v_remove(struct spi_device *spi)
{
	...
	free_irq(gpiod_to_irq(drm->detect), drm->spi);
	devm_gpiod_put(&spi->dev, drm->detect);
...
MODULE_DESCRIPTION("drm hotplug interrupt test");
