...
static int drm_st7789v_probe(struct spi_device *spi)
{
	...
	if(device_property_read_u32(dev, "width", &width) ||
		...
		if((hd > 0) && (vd > 0) && (hd_mm > 0) && (vd_mm > 0)) { //获取模块参数
			st7789v_mode.hdisplay = hd;
			st7789v_mode.vdisplay = vd;
			st7789v_mode.width_mm = hd_mm;
			st7789v_mode.height_mm = vd_mm;
		...
		st7789v_mode.hdisplay = width;
		st7789v_mode.vdisplay = height;
		st7789v_mode.width_mm = width_mm;
		st7789v_mode.height_mm = height_mm;
