...
struct dummy_group_desc{
	const char *name;
	const unsigned int *pins;
	const unsigned int npins;
};

static const unsigned int uart_pins[] = {0, 1}; //UART引脚
static const unsigned int i2c_pins [] = {2, 3}; //I2C引脚
static const unsigned int spi_pins [] = {4, 5, 6, 7};  //SPI引脚
static const unsigned int usb_pins [] = {8, 9};  //USB引脚

static const struct dummy_group_desc dummy_pinctrl_groups[] =
{
	{ .name = "uart_group", .pins = uart_pins, .npins = ARRAY_SIZE(uart_pins) },
    { .name = "i2c_group", .pins = i2c_pins, .npins = ARRAY_SIZE(i2c_pins) },
    { .name = "spi_group", .pins = spi_pins, .npins = ARRAY_SIZE(spi_pins) },
	{ .name = "usb_pins", .pins = usb_pins, .npins = ARRAY_SIZE(usb_pins) },
};

static int dummy_get_groups_count(struct pinctrl_dev *pctldev)
{
    int count = ARRAY_SIZE(dummy_pinctrl_groups);
    printk(KERN_INFO "groups_count: %d\n", count);
	return ARRAY_SIZE(dummy_pinctrl_groups);
}

static const char *dummy_get_group_name(struct pinctrl_dev *pctldev,unsigned selector)
{
    printk(KERN_INFO "groups_name[%d]: %s\n", selector, dummy_pinctrl_groups[selector].name);
	return dummy_pinctrl_groups[selector].name;
}

static int dummy_get_group_pins(struct pinctrl_dev *pctldev, unsigned selector, const unsigned **pins,unsigned *num_pins)
{
	*pins = dummy_pinctrl_groups[selector].pins;
	*num_pins = dummy_pinctrl_groups[selector].npins;
	return 0;
}
...
static int dummy_set_mux(struct pinctrl_dev *pctldev, unsigned func_selector, unsigned group_selector)
{
	...
	volatile int i = 0;
	uint32_t value;
	for(i = 0; i < dummy_pinctrl_groups[group_selector].npins; i++)
	{
		pin = dummy_pinctrl_groups[group_selector].pins[i];
		//设置MUX寄存器，以选中相应的功能
